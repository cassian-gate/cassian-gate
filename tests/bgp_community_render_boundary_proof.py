#!/usr/bin/env python3
"""bgp_community_render_boundary_proof.py -- §4.10 WI-3 RENDER-1 proof (P6/P7).

DC §13(b)(c) failed-invariant render-boundary symmetry for bgp_community, lab-free
(SP #1 render-boundary pattern; mirrors udi_render_boundary_proof /
bl6_observed_state_absence_render_proof). Drives cassian_tests._format_test_summary
with synthetic results; cassian_tests.py is byte-unchanged (the render surface is
type-generic: it branches on isinstance(observed_state, dict), not inv_type).
...
(PBE-1b-1):
  ...
  P6  PRESENT-HALF  data-present-match-failed: a kind:invariant verdict:fail
      record with a DICT observed_state (route_present:true, communities present
      but not matching expected) -> _format_observed_state_block: explicit
      `observed:` block surfacing the bgp_community fields.
  P7  ABSENT-HALF   data-absent: a kind:invariant verdict:fail record with a
      ...
      non-dict state (every fail emits a dict), so it is proven synthetically.
...
def _results():
    return {
        "lab": "bgp-community-render-boundary-proof",
        "result": "fail",
        "summary": {"total": 4, "passed": 1, "failed": 3},
        "tests": [
            # P6 PRESENT-HALF: dict observed_state, data-present-match-failed
            # (route present; communities present but not matching expected).
            {
                "name": "bc-present",
                "kind": "invariant",
                "from": "r2",
                ...
                "observed": "fail",
                "verdict": "fail",
                "error": "bgp_community mismatch (expected pass, observed fail)",
                "meta": {"type": "bgp_community", "prefix": "1.1.1.1/32",
                         "route_present": True, "observed_community_count": 2},
                "observed_state": {
                    "type": "bgp_community",
                    "prefix": "1.1.1.1/32",
                    "expected_communities": ["65000:100", "internet"],
                    "actual_communities": ["65111:9", "no-export"],
                    "match": "all",
                    "route_present": True,
                    "source_node": "r2",
                },
            ...
            # meta carries a runtime-variant key (observed_community_count) that
            # must NOT enter the rendered absence surface (D06).
            {
                "name": "bc-absent",
                "kind": "invariant",
                "from": "r2",
                ...
                "error": "bgp_community mismatch (expected pass, observed fail)",
                "meta": {"type": "bgp_community", "prefix": "1.1.1.1/32",
                         "observed_community_count": 5},
            },
            # passing bgp_community invariant: not a failure -> not in failed_tests.
            {
                "name": "bc-pass",
                "kind": "invariant",
                "from": "r2",
...
def main():
    ...
    check("P6 present failed line present", " - bc-present (invariant) r2->" in out1)
    check("P6 present block renders actual_communities",
          "actual_communities: ['65111:9', 'no-export']" in out1)
    check("P6 present block renders expected_communities",
          "expected_communities: ['65000:100', 'internet']" in out1)
    check("P6 present block renders match", "match: all" in out1)
    ...
    check("P6 present record does NOT render the unavailable indicator (dict -> present path)",
          out1.split("bc-present")[1].split("bc-absent")[0].find("structured failure detail unavailable") == -1)

    # ---- P7 ABSENT-HALF (data-absent -> explicit (a)/(b)/(c) indicator) ----
    check("P7 absent failed line present", " - bc-absent (invariant) r2->" in out1)
    check("P7 (iv) explicit unavailable indicator renders", UNAVAILABLE in out1)
    check("P7 (a) invariant type present", "type: bgp_community" in out1)
    ...
    check("P7 (D06) runtime-variant meta excluded", "observed_community_count" not in out1)

    # ---- distinctness (PBE-1b-1): two halves, two observed blocks, distinct inputs ----
