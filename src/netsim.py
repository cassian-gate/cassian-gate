...
def cmd_preflight(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        report = _preflight_report(input_ref=input_ref, topo_path=topo_path, resolved=resolved, cov=cov)

        if fmt == "json":
            _preflight_write(out_path, report)
        else:
            out_path.parent.mkdir(parents=True, exist_ok=True)
            out_path.write_text(_preflight_format_text(report), encoding="utf-8")
