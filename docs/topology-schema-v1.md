...
A SONiC network OS running as a VM guest.

Properties:

* VM runtime (`runtime: vm`); you supply a vrnetlab-built image
* routing configuration is either generated by Cassian Gate or supplied whole by you
* see `vm-runtime-capabilities.md` for the supported surface and its current boundaries
...
- name: s1
  ...
  sonic_mode: generated | preconfigured
  sonic_config_db: <path, required iff preconfigured>
```

#### `sonic_mode: generated` (default)

* Cassian Gate generates a `config_db.json` overlay from the topology
* the overlay carries only Cassian-declared tables; platform-owned tables
  (`PORT`, `hwsku`, `platform`, `mac`) are never written
* `sonic_config_db` must **not** be declared in this mode — declaring it is rejected

---

#### `sonic_mode: preconfigured`

* you supply a **complete** `config_db.json` — the whole file, platform tables included
* `sonic_config_db` is **required**, and names that file
* the artifact must parse as JSON and carry `PORT` and `DEVICE_METADATA`
* a relative path resolves against **the topology file's own directory**, never the
  working directory; a relative path with no topology file on disk is rejected
* no generation is invoked for the node

A missing artifact is rejected before any guest is contacted, naming the node,
the declared mode, the missing path, and your two options — supply the artifact,
or switch to `generated`.
